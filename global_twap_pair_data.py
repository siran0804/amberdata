...
BASE_URL = "https://web3api.io/api/v2/market/spot/twap/pairs/"
SELECT_SQL = '''select * from spot_market_data.global_twap_pair_infomation where startdate is not null'''

INSERT_HISTORICAL_SQL = '''INSERT INTO spot_market_data.global_twap_pair_data (pair,tm,twap,
price,volume,timeInterval) VALUES (%(pair)s, %(timestamp)s, %(twap)s, %(price)s,
...
def insert_global_twap_pair_data(pair, start, end, timeinterval):
...
tm_sql = '''select tm from spot_market_data.global_twap_assets_data where pair=\'{}\' order by tm desc limit 1;'''


def global_twap_pair_data_run(history_or_now, timeinterval):
    for row in rows:
        ...
        for i in range(len(days_list) - 1):
            ...
            if timeinterval == "minutes":
                insert_global_twap_pair_data(row[0], start, end, 'minute')
            if timeinterval == "hours":
                insert_global_twap_pair_data(row[0], start, end, 'hour')
            if timeinterval == "days":
                insert_global_twap_pair_data(row[0], start, end, 'day')
...
if __name__ == '__main__':
    ...
    for timeinterval in timeinterval_list:
        global_twap_pair_data_run(history_or_now, timeinterval)
    conn.close()