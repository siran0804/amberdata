...
BASE_URL = "https://web3api.io/api/v2/market/spot/prices/pairs/"
SELECT_SQL = '''select * from spot_market_data.price_pair_infomation where startdate is not null'''

INSERT_HISTORICAL_SQL = '''INSERT INTO spot_market_data.price_pair_data (pair,tm,
price,volume,timeInterval) VALUES (%(pair)s, %(timestamp)s, %(price)s,
...
def insert_price_pair_data(asset, start, end, timeinterval):
...
tm_sql = '''select tm from spot_market_data.price_pair_data where pair=\'{}\' order by tm desc limit 1;'''


def price_pair_data_run(history_or_now, timeinterval):
    for row in rows:
        ...
        for i in range(len(days_list) - 1):
            ...
            if timeinterval == "minutes":
                insert_price_pair_data(row[0], start, end, 'minute')
            if timeinterval == "hours":
                insert_price_pair_data(row[0], start, end, 'hour')
            if timeinterval == "days":
                insert_price_pair_data(row[0], start, end, 'day')
...
if __name__ == '__main__':
    ...
    for timeinterval in timeinterval_list:
        price_pair_data_run(history_or_now, timeinterval)
    conn.close()